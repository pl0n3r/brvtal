...
deployment_expect(str_contains($resolver, 'function brvtal_deployment_is_exact()'), 'resolver must distinguish exact environment/git source from release fallback');
deployment_expect(str_contains($endpoint, "'short_commit'"), 'public deployment endpoint must expose the deployed short SHA');
deployment_expect(str_contains($endpoint, "'exact' => brvtal_deployment_is_exact()"), 'deployment endpoint must identify whether SHA is exact');
