...
totp_enrollment_expect(str_contains($login, 'const data = await r.clone().json()'), 'login challenge must inspect a clone without consuming the real verification response');
totp_enrollment_expect(str_contains($login, 'return verifiedResponse;'), 'login challenge must preserve the real server response for Safari compatibility');
totp_enrollment_expect(!str_contains($login, "return new Response(JSON.stringify(verified), {status:200"), 'login challenge must not reconstruct a successful response in the browser');
