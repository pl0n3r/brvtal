...
function transmissions_expect(bool $condition, string $message): void {
    if (!$condition) {
        fwrite(STDERR, "PUBLIC TRANSMISSIONS CONTRACT FAILED: {$message}\n");
        exit(1);
    }
}
...
transmissions_expect(str_contains($script, "event:'events',artist:'artists',set:'sets',release:'releases'"), 'API singular relation types must map to canonical routes');
transmissions_expect(str_contains($script, 'related_id'), 'relations must resolve through API related_id references');
transmissions_expect(str_contains($script, 'data?.archive?.events'), 'archived Events must remain resolvable from Blog relations');
