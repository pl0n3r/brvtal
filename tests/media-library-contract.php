...
media_assert(str_contains($api, 'brvtalMediaRemoveGeneratedVariants'), 'regeneration must clean previously tracked variants');
...
media_assert(
    str_contains($mediaConfig, "\$specs['display'] = [")
        && str_contains($mediaConfig, 'brvtalMediaVariantSpecs'),
    'Media Engine must define the generic display WebP through the v3 variant policy'
);
...
media_assert(
    str_contains($mediaConfig, 'function brvtalMediaOriginalIdentity(')
        && str_contains($mediaConfig, "'path' => brvtal_media_public_upload_path(\$absoluteOriginal)"),
    'Media Engine must keep the uploaded original authoritative through the v3 identity boundary'
);
media_assert(str_contains($mediaConfig, 'BRVTAL_MEDIA_VARIANT_POLICY_VERSION = 3'), 'Media Engine incremental policy must be versioned');
media_assert(str_contains($mediaConfig, 'BRVTAL_MEDIA_WEBP_QUALITY = 82'), 'Media Engine WebP quality must be traceable');
media_assert(str_contains($mediaConfig, 'brvtalMediaVariantDecision'), 'Media Engine must decide reuse/repair/regeneration per physical variant');
media_assert(str_contains($mediaConfig, "'alias_of' => 'display'"), 'equivalent w1920 output must alias display instead of duplicating an encode');
media_assert(str_contains($mediaConfig, "'physical_bytes'"), 'sidecars must expose generated physical footprint');
media_assert(str_contains($mediaConfig, "'reused_at'"), 'reused variants must retain distinct reuse provenance');
media_assert(str_contains($mediaConfig, "'state' => 'alias'"), 'logical aliases must be explicit in sidecar metadata');
media_assert(str_contains($mediaConfig, "'rolled_back'"), 'partial optimization must record rolled-back derivatives instead of pretending they were committed');
media_assert(str_contains($mediaConfig, 'brvtalMediaRollbackStagedVariants'), 'variant commit failures must restore previous public derivatives');
media_assert(str_contains($mediaConfig, "'_staged_files'"), 'new encodes must remain private until sidecar commit succeeds');
...
media_assert(str_contains($adminModules, "['media', ['media']]") && str_contains($adminModules, 'sectionDependencies.has(section)') && str_contains($adminModules, 'prepareModuleWorkspace(section)'), 'MEDIA navigation must be intercepted through the shared dynamic module workspace');
...
media_assert(str_contains($controller, 'OPTIMIZATION /'), 'Media inspector must expose optimization state');
media_assert(str_contains($controller, 'PHYSICAL FOOTPRINT'), 'Media inspector must expose generated byte footprint');
media_assert(str_contains($controller, 'Optimization up to date · existing variants reused.'), 'focal regeneration must not pretend reused variants were freshly encoded');
