...
26. **Every deploy README is a professional development dashboard and must preserve situational awareness.** It should make the current work state, Git delta, quality gates, next action and project-wide pending panorama scannable in seconds. Useful development information takes priority over decorative branding; do not make a large logo the dominant README element. The panorama is not a changelog and must be revised as work is completed or reprioritized.
...
39. **Observe Hostinger deployment in parallel with exact-main CI.** A lightweight main-push workflow may poll the canonical public deploy marker for the exact pushed SHA while BRVTAL CI is still running. Marker observation means only **DEPLOYED marker observed**; it never means **VALIDATED IN PRODUCTION**, and production performance/smoke remain separate evidence.
...
1. **Delivery lead-time optimization — #534** — finish preflight fan-out, atomic Git write policy, concurrent final gates and parallel deploy observation; measure the result.
