...
4. **DISCADMIN simplification/stabilization** — fix concrete friction or duplication while preserving destination-based navigation and internal Content Core architecture.
5. **Incremental Hero Slider improvements** — only when they add real editing value while preserving fallback/mobile/performance.
6. **Analytics/privacy maturity** proportional to real product needs.
7. **Performance/recovery maintenance** — keep exact-deploy modern Chromium evidence and isolated backup recovery rehearsal green; re-optimize/rework only when measured evidence changes, and never enable automatic production restore.
