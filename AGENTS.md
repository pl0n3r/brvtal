...
53. **Roadmaps are evidence-rich operational ledgers, not disposable checklists.** Organize durable planning by phases and keep every meaningful completed milestone visible in place. Use `✅ ~~completed item~~` only after its required gates; when useful, append the product version, linked PR, merge SHA, exact-main CI/run evidence, deploy observation and production validation evidence in the same bullet or its immediate continuation. Use `🚧 active/pending item` without strikethrough and include the current PR/head/gate state when work is in flight. Use `⛔ blocked item` with the concrete blocker and the next evidence/action needed. Never delete a completed roadmap item merely to shorten the issue; compress wording instead. Never leave an item marked in-flight after merge/closeout. CI success, deployment observation and production validation must remain explicitly distinct. The canonical roadmap Issue and any phase tracker should visually read like a chronological execution ledger, matching the phase + status + linked-evidence pattern used in #533.
