...
The owner-facing sidebar has two high-level groups and one stable primary order:
...
Global Search stays available from the top-right control and from a persistent sidebar affordance immediately above Logout; both open the same canonical search experience. Content Health, Bulk Actions, feedback and appearance enhance the shell or relevant screens instead of becoming competing top-level mental models.
...
The Home Hero Slider (owner-facing label: **Banners**) is a constrained BRVTAL-specific, LayerSlider-inspired module inside DISCADMIN. It is **not** a generic Wix/Elementor page builder.
