...
function brvtal_blog_sanitize_html(string $html): array
{
    ...
    $safeUrl = static function (string $url, bool $image = false): bool {
        ...
        if (
            !$image
            && (str_starts_with($url, '#') || str_starts_with(strtolower($url), 'mailto:'))
        ) {
            return true;
        }
    ...
    if (!$loaded) {
        return ['html' => '', 'warnings' => ['Body HTML could not be parsed and was removed.']];
    }
        ...
        foreach (iterator_to_array($node->childNodes) as $child) {
            ...
            foreach (iterator_to_array($child->attributes ?? []) as $attr) {
                ...
                if (
                    ...
                    || !in_array($name, $allowedAttrs[$tag] ?? [], true)
                ) {
                ...
                if ($name === 'style') {
                    ...
                    if ($clean !== trim($attr->value)) {
                        $warnings['style'] = 'Unsupported inline styles were removed.';
                    }
                }

                $isLinkUrl = $tag === 'a' && $name === 'href';
                $isImageUrl = $tag === 'img' && $name === 'src';
                if (($isLinkUrl || $isImageUrl) && !$safeUrl($attr->value, $isImageUrl)) {
