...
function brvtal_deployment_sha(): string
{
    ...
    if (preg_match('/^[a-f0-9]{7,40}$/i', $environment)) {
...
{
    $environment = trim((string)getenv('BRVTAL_DEPLOY_COMMIT'));
    if (preg_match('/^[a-f0-9]{7,40}$/i', $environment)) return 'environment';
    if (is_file(dirname(__DIR__) . '/.git/HEAD')) return 'git_checkout';
