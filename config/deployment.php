...
function brvtal_deployment_sha(): string
{
    ...
    if (is_string($head)) {
        ...
        if (preg_match('/^[a-f0-9]{40}$/i', $head)) return $resolved = strtolower($head);
    }
...
{
    $environment = trim((string)getenv('BRVTAL_DEPLOY_COMMIT'));
    if (preg_match('/^[a-f0-9]{40}$/i', $environment)) return 'environment';

    $gitDirectory = dirname(__DIR__) . '/.git';
    $head = trim((string)@file_get_contents($gitDirectory . '/HEAD'));
    if (preg_match('/^[a-f0-9]{40}$/i', $head)) return 'git_checkout';
    if (preg_match('/^ref:\s+(.+)$/', $head, $match)) {
        $reference = trim((string)@file_get_contents($gitDirectory . '/' . ltrim($match[1], '/')));
        if (preg_match('/^[a-f0-9]{40}$/i', $reference)) return 'git_checkout';
        $packed = @file($gitDirectory . '/packed-refs', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [];
        foreach ($packed as $line) {
            if ($line === '' || $line[0] === '#' || $line[0] === '^') continue;
            [$sha, $ref] = array_pad(preg_split('/\s+/', trim($line), 2) ?: [], 2, '');
            if ($ref === $match[1] && preg_match('/^[a-f0-9]{40}$/i', $sha)) return 'git_checkout';
        }
    }
...
function brvtal_release_identity(): string
{
    return 'v' . BRVTAL_APP_VERSION;
}

function brvtal_deployment_cache_key(): string
{
    if (brvtalDeploymentIsExact()) return brvtal_deployment_short_sha();
