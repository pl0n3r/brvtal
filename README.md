...
- `mutationLabels()` deja de depender de una cadena larga de condicionales y pasa a reglas declarativas ordenadas.
- Se preserva exactamente la prioridad actual por endpoint, acción y método para Media Library, Releases, Blog, TOTP, DELETE genérico, Settings y fallback general.
- Los textos visibles de progreso y éxito no cambian.
- La regresión Playwright verifica la precedencia para Media upload, Releases delete, Blog save, Settings save, DELETE genérico y save genérico sin depender de temporización de red.
...
- Esa base pasó **BRVTAL CI #978** y queda **VALIDATED IN CODE**.
...
- Production Performance reciente no constituye validación de producción cuando el marcador exacto de deploy de Hostinger no fue observado.

## Qué sigue

- Cerrar este bloque de #451 y tacharlo en el umbrella cuando el merge y el CI del `main` exacto queden verdes.
...
- Revisar el bloque Sonar de `Math.random()` ya anotado en #451, clasificando cada uso antes de modificarlo.

## Panorama general pendiente

- **Sonar / calidad:** #451 continúa por riesgo y área; P1 ya quedó cubierto o dejó de reproducirse y los P2 se resuelven en PRs pequeños.
