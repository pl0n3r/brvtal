...
- Tablet simplifica la retícula a seis columnas; móvil vuelve a una secuencia estable de una columna, con targets táctiles de 44 px y sin overflow horizontal.
...
- `css/public-media.css` — sustituye la grilla uniforme por la retícula editorial responsive, estiliza controles/tarjetas/fallback y conserva viewer/reduced-motion.
...
- La implementación está lista para validación dirigida por `BRVTAL CI`; no se declara validación local que no se haya ejecutado.
- Pendiente en este snapshot: `BRVTAL CI / validate` y revisión automática del head final del PR.
