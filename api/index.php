...
try {
  ...
  json_response(['ok'=>true,'admin'=>['id'=>(int)$a['id'],'name'=>$a['name'],'email'=>$a['email']],'csrf'=>brvtal_admin_csrf_token()]);
    ...
    if($method==='POST') {
        $d=input_json(); if($resource==='settings'){ $keyState=brvtal_setting_key_normalize((string)($d['setting_key']??''));if($keyState['error']!==null)json_response(['ok'=>false,'error'=>$keyState['error']['error'],'field'=>$keyState['error']['field']],422);$key=(string)$keyState['key'];if($key===''||strlen($key)>120)json_response(['ok'=>false,'error'=>'KEY_REQUIRED'],422);if($key==='security.totp_encryption_key')json_response(['ok'=>false,'error'=>'PROTECTED_SETTING'],403);$value=(string)($d['setting_value']??'');$themeSettingError=brvtal_theme_setting_error($key,$value);if($themeSettingError!==null)json_response(['ok'=>false,'error'=>$themeSettingError['error'],'field'=>$themeSettingError['field']],422);$themeReferenceError=brvtal_theme_active_reference_error($key,$value,static function(string $slug) use($pdo): bool {$st=$pdo->prepare('SELECT setting_value FROM settings WHERE setting_key=? AND is_json=1 LIMIT 1');$st->execute(['theme.'.$slug]);$raw=$st->fetchColumn();if(!is_string($raw))return false;$decoded=json_decode($raw,true);return is_array($decoded);});if($themeReferenceError!==null)json_response(['ok'=>false,'error'=>$themeReferenceError['error'],'field'=>$themeReferenceError['field']],422);if(strlen($value)>2*1024*1024)json_response(['ok'=>false,'error'=>'VALUE_TOO_LARGE'],422);if((int)($d['is_json']??0)===1&&json_decode($value,true)===null&&strtolower(trim($value))!=='null')json_response(['ok'=>false,'error'=>'INVALID_SETTING_JSON'],422);$st=$pdo->prepare('INSERT INTO settings(setting_key,setting_value,is_json) VALUES(?,?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value),is_json=VALUES(is_json)');$st->execute([$key,$value,(int)($d['is_json']??0)]);json_response(['ok'=>true]); }
    ...
    if($method==='DELETE'&&$resource==='settings'&&$id===null){$key=preg_replace('/[^a-zA-Z0-9_.-]/','',(string)($_GET['key']??''))??'';if($key==='')json_response(['ok'=>false,'error'=>'KEY_REQUIRED'],422);if($key==='security.totp_encryption_key')json_response(['ok'=>false,'error'=>'PROTECTED_SETTING'],403);$activeTheme=$pdo->query("SELECT setting_value FROM settings WHERE setting_key='theme.active' LIMIT 1")->fetchColumn();$themeDeleteError=brvtal_theme_delete_reference_error($key,is_string($activeTheme)?trim($activeTheme):null);if($themeDeleteError!==null)json_response(['ok'=>false,'error'=>$themeDeleteError['error'],'field'=>$themeDeleteError['field']],409);$st=$pdo->prepare('DELETE FROM settings WHERE setting_key=?');$st->execute([$key]);json_response(['ok'=>true,'deleted'=>(int)$st->rowCount()]);}
    method_not_allowed();
