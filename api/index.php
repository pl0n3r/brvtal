  ...
  json_response(['ok'=>true,'admin'=>['id'=>(int)$a['id'],'name'=>$a['name'],'email'=>$a['email']],'csrf'=>brvtal_admin_csrf_token()]);
    ...
    if($method==='PUT'&&$id!==null){
        ...
        try{
            ...
            if($resource==='events'){$p=brvtal_event_lifecycle_patch($before,$p);$eventStateError=brvtal_event_publication_error(array_replace($before,$p));if($eventStateError!==null){$pdo->rollBack();json_response(['ok'=>false,'error'=>$eventStateError['error'],'field'=>$eventStateError['field']],422);}}
