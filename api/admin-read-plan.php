...
/**
 * Fetch a paginated Sets page with late materialization.
 *
 * The ordering query touches only id + sort_order, so MariaDB does not need
 * to filesort wide TEXT/URL rows before LIMIT/OFFSET. Full records are loaded
 * only for the selected page and then restored to the canonical order.
 *
 * @param array{where_sql:string,params:array<int,string>} $pagePlan
 * @param array{page_size:int,offset:int} $pagination
 * @return array<int,array<string,mixed>>
 */
function brvtalAdminFetchSetsPage(PDO $pdo, array $pagePlan, array $pagination): array
{
    $whereSql = (string)($pagePlan['where_sql'] ?? '');
    $params = is_array($pagePlan['params'] ?? null) ? $pagePlan['params'] : [];
    $limit = max(1, min(BRVTAL_ADMIN_COLLECTION_PAGE_SIZE_MAX, (int)($pagination['page_size'] ?? BRVTAL_ADMIN_COLLECTION_PAGE_SIZE)));
    $offset = max(0, (int)($pagination['offset'] ?? 0));

    $idStatement = $pdo->prepare(
        "SELECT id FROM sets_media{$whereSql} ORDER BY sort_order ASC,id ASC LIMIT {$limit} OFFSET {$offset}"
    );
    $idStatement->execute($params);
    $ids = array_values(array_filter(
        array_map('intval', $idStatement->fetchAll(PDO::FETCH_COLUMN)),
    ...
    $rowStatement = $pdo->prepare("SELECT * FROM sets_media WHERE id IN ({$placeholders})");
    $rowStatement->execute($ids);
    $rowsById = [];
    foreach ($rowStatement->fetchAll() as $row) {
