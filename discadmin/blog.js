window.BRVTALBlog = (() => {
  ...
  function payload(existingRelations=[]){const relations=mergeRelations(existingRelations);return{title:value('blog_title'),slug:value('blog_slug'),excerpt:value('blog_excerpt'),body:value('blog_body'),cover_image:normalizeMediaPath(value('blog_cover_image')),seo_title:value('blog_seo_title'),seo_description:value('blog_seo_description'),status:value('blog_status_field')||'draft',featured:input('blog_featured')?.checked?1:0,sort_order:Number(value('blog_sort_order')||0),relations}}
