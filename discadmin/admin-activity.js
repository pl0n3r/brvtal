(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .activity-pagination{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:12px;font:800 9px/1 monospace;color:#858d93}
  ...
  async function fetchList(resource = '', cursor = null) {
  ...
  async function fetchHistory(resource, resourceId, cursor = null, options = {}) {
  ...
  function render(data, selectedResource = '', loadId = listLoadId) {
    ...
    panel.querySelector('[data-activity-load-more]')?.addEventListener('click', async event => {
      const button = event.currentTarget;
      if (!data?.next_cursor || button.disabled) return;
