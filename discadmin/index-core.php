...
</style><link rel="stylesheet" href="/discadmin/admin-modules.css?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"><link rel="stylesheet" href="/discadmin/content-core.css?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"><link rel="stylesheet" href="/discadmin/security.css?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></head><body><div id="app"></div>
<div class="modal" id="modal"><div class="modalbox"><div class="modalhead"><div><h2 id="mtitle"></h2><div class="micro">BRVTAL / DISCADMIN</div></div><button class="iconbtn" onclick="closeModal()">ESC</button></div><div id="notice" class="notice"></div><div id="mcontent"></div><div class="modalfoot"><span class="helper">Cambios guardados directamente en MySQL.</span><div class="footactions"><button class="btn ghost" onclick="closeModal()">CANCELAR</button><button class="btn red" id="saveBtn">GUARDAR</button></div></div></div></div>
<script>
...
function field(id,label,val='',type='text',full=false){return `<div class="field ${full?'full':''}"><label>${label}</label><input id="f_${id}" type="${type}" value="${esc(val)}"></div>`}
function area(id,label,val='',full=true){return `<div class="field ${full?'full':''}"><label>${label}</label><textarea id="f_${id}">${esc(val)}</textarea></div>`}
function select(id,label,val,opts){return `<div class="field"><label>${label}</label><select id="f_${id}">${opts.map(o=>`<option value="${o[0]}" ${o[0]===val?'selected':''}>${o[1]}</option>`).join('')}</select></div>`}
...
scheduleTechRefresh();
</script><script src="/discadmin/qrcode.min.js?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></script><script src="/discadmin/content-core-lineup.js?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></script><script src="/discadmin/content-core.js?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></script><script src="/discadmin/security.js?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></script><script src="/discadmin/admin-modules.js?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></script><script src="/discadmin/totp-login.js?v=<?= rawurlencode(BRVTAL_APP_BUILD) ?>"></script><script>restoreSession();</script></body></html>