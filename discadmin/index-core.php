...
function render(){const root=document.getElementById('app');if(!state.authed){root.innerHTML='<div class="login"><form class="loginbox" onsubmit="login(event)"><h1>BRVTAL</h1><div class="sub">DISCADMIN / CONTROL ROOM</div><div class="syscheck"><div class="sysrow"><span>WEB / PHP</span><b class="sysok">ONLINE</b></div><div class="sysrow"><span>API</span><b id="st_api" class="syswait">CHECKING</b></div><div class="sysrow"><span>DATABASE</span><b id="st_db" class="syswait">CHECKING</b></div><div class="sysrow"><span>SESSION</span><b class="sysok">READY</b></div></div><label class="eyebrow">EMAIL</label><input name="email" type="email" required><label class="eyebrow">PASSWORD</label><input name="password" type="password" required><button class="btn" type="submit" style="width:100%">ENTER</button><div class="error"></div></form></div>';setTimeout(checkSystem,300);return}root.innerHTML=`<div class="shell"><aside class="side"><div class="logo">BRVTAL</div><div class="sub">DISCADMIN / CONTROL</div><div class="nav">${['dashboard','events','artists','sets','media','pages','theme','settings'].map(x=>`<button class="${state.section===x?'active':''}" onclick="go('${x}')">${x==='theme'?'THEME STUDIO':x.toUpperCase()}</button>`).join('')}<div class="navgroup">TECHNICAL</div>${[['system','SYSTEM STATUS']].map(x=>`<button class="${state.section===x[0]?'active':''}" onclick="tech('${x[0]}')">${x[1]}</button>`).join('')}</div><div class="sidefoot"><button class="btn ghost" style="width:100%" onclick="logout()">LOG OUT</button></div></aside><main class="main"><div class="top"><div><div class="eyebrow">BRVTAL CMS</div><h1>${state.section.toUpperCase()}</h1></div><span class="status"><i></i>ONLINE</span></div>${content()}</main></div>`}
...
scheduleTechRefresh();restoreSession();
</script><script src="/discadmin/totp-login.js"></script></body></html>